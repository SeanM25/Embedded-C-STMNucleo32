 ...
 *
 * This file provides prototypes for functions defined in Leds.c, which is 
 * appropriate for Lab08 ONLY. Future labs will use a header-only version of 
 * the file.
 */
 ...
 * 
 * After "calling" LEDS_INIT(), the other functions in this file can be used 
 * to manipulate the LED bar.
 */
 ...
 * last 7 LEDs off.  LEDS_SET(0x1F) should turn off the first three LEDs and
 * turn on the remaining five LEDs. 
 *
 * @param newPattern: An 8-bit value describing the desired output on the 
 *                    LED bar. 
 *
...
/**
 * LEDS_GET() evaluates to the current state of the LED bar.  "Return" values 
 * are 8-bit patterns, where each bit describes the current state of the 
 * corresponding LED So, for example, if the  led bar's led's are [ON OFF ON 
 * OFF   OFF ON OFF ON], LEDS_GET() should "return" 0xA5. 
 *
