...
int main()
{
    ...
    // Configure Timer 2 using PBCLK as input. We configure it using a 1:16 
    // prescalar, so each timer 
    // tick is actually at F_PB / 16 Hz, so setting PR2 to F_PB / 16 / 100 
    // yields a .01s timer
    ...
    // Configure Timer 3 using PBCLK as input. We configure it using a 1:256 
    // prescaler, so each timer tick is actually at F_PB / 256 Hz, so setting 
    // PR3 to F_PB / 256 / 5 yields a .2s timer.
